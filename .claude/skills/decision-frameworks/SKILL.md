...
03b. **Check precedent before recommending; do not infer a leaning.** `decisions-observed.md` (private layer) records real decisions with their reasoning, each tagged by how much evidence stands behind it. Most entries are a **single instance** — surface them as "something similar came up on this date, here is what happened", never as "he typically does X". Only entries tagged `[stated]` or `[corroborated]` may be applied as rules. Where the data does not settle a question, the supported behaviour is to say so and hand the question over, not to pick a number.
