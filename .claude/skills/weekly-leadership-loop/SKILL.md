...
1. **Pull status across pillars/wards.** Agent reads `org-profile.md`, latest dashboards/reports, and `ops-history.md` to draft a pre-read: what's on track, what's at risk, what's new since last week.
...
7. **Log to `ops-history.md`.** Every decision, experiment, or notable status change from the meeting gets a row — this is what makes next week's pre-read sharper than this week's.
...
- "Draft this week's Leadership Team pre-read from the knowledge base and `ops-history.md`."
